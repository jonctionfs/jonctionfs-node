import { OauthClient, OauthTokens, OauthResponse } from '../../oauth.js'
...
export const createConnection = async (provider: Provider, userId: string) => {
  ...
  const connection: FileSystemAPI = {
    async get(obj): Promise<Obj> {
      ...
      return {
        contentUrl: (res as OauthResponse)?.body?.webViewLink as string,
        type: 'object'
      }
    },
    ...
    async upload(obj, parents){
      ...
      return {
        contentUrl: process.env.UPLOAD_SERVER_URL + '/upload/google-drive/' + res.headers?.get('Location'),
        method: 'POST'
      }
    },
